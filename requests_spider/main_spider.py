...
def main():
    ...
    try:
        ...
        logging.info("MySQL 数据库连接成功，数据表已准备好！")
    except Exception as e:
        logging.error(f"MySQL 连接或建表失败: {e}")
        return  # 连不上数据库就不往下跑了
        ...
        try:
            ...
            for movie in movies_base_info:
                ...
                INSERT INTO douban_top250 (
                    rank_num, title_cn, title_other, cast_info, rating, rating_count, 
                    quote, detail_url, year_info, runtime, genre, imdb, hot_comments
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """
                # 注意顺序要和上面的 SQL 字段顺序一致
                values = (
                    movie['排名'], movie['中文标题'], movie['外文标题'], movie['导演/主演'], 
                    movie['评分'], movie['评价人数'], movie['简介'], movie['详情链接'], 
                    movie['年份'], movie['片长'], movie['类型'], movie['IMDb'], movie['热门短评']
                )
                try:
                    cursor.execute(insert_sql, values)
                    db.commit()  # 每抓完一条就提交保存
                except Exception as e:
                    logging.error(f"存入 MySQL 失败: {e}")
                    db.rollback()
